...
const sBTCBytes32 = '0x7342544300000000000000000000000000000000000000000000000000000000';
...
describe('keeper', () => {
  ...
  test('run', async () => {
    const PositionLiquidatedMock = jest.fn().mockReturnValue('__PositionLiquidated_EVENT_FILTER__');
    const PositionModifiedMock = jest.fn().mockReturnValue('__PositionModified_EVENT_FILTER__');
    const FundingRecomputedMock = jest.fn().mockReturnValue('__FundingRecomputed_EVENT_FILTER__');
    ...
    const arg = {
      ...
      futuresMarket: {
        ...
        assetPrice: jest.fn().mockResolvedValue({ price: BigNumber.from(100), invalid: false }),
      },
      signerPool: jest.fn(),
  ...
  test('updateIndex', async () => {
    ...
    const arg = {
      baseAsset: 'sBTC',
      futuresMarket: {
        assetPrice: jest.fn().mockResolvedValue({ price: price, invalid: false }),
      },
      signerPool: jest.fn(),
      provider: jest.fn(),
    } as any;
  ...
  test('runKeepers', async () => {
    ...
    const liquidateOrderSpy = jest.spyOn(keeper, 'liquidateOrder').mockImplementation();
    ...
    expect(futuresOpenPositionsSetMock).toHaveBeenCalledWith({ market: 'sUSD' }, 4);
  ...
  test('liquidateOrder bails when it cant liquidate', async () => {
    const arg = {
      ...
      futuresMarket: {
        ...
        liquidationPrice: jest.fn().mockResolvedValue({ price: utils.parseEther('1') }),
  ...
  test('liquidation position order', async () => {
    ...
    expect(out2.map(group => group.map(p => p.account))).toEqual([['a'], ['b', 'c', 'd'], ['f']]);
  });
});
